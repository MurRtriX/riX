...
iptables -t nat -I PREROUTING -i $(ip -4 route ls|grep default|grep -Po '(?<=dev )(\S+)'|head -1) -p udp --dport 53 -j DNAT --to-destination :53
ip6tables -I INPUT -p udp --dport 53 -j ACCEPT
ip6tables -t nat -I PREROUTING -i $(ip -4 route ls|grep default|grep -Po '(?<=dev )(\S+)'|head -1) -p udp --dport 53 -j DNAT --to-destination :53
...
[Service]
Type=forking
ExecStart=/usr/bin/screen -dmS dnstt /bin/dnstt-linux-amd64 -udp :53 -privkey-file /usr/bin/server.key $ns 127.0.0.1:443
Restart=always
User=root
